...
namespace ConferenceManagementSystem.Controllers
{
    public class UserController : Controller
    {
        ...
        public ActionResult Create([Bind(Include = "UserId,Email,Username,Password,ConfirmedPassword,TitleId,FullName,GenderId,Instituition,Faculty,Department,ResearchField,Address,State,PostalCode,CountryId,PhoneNumber,FaxNumber")] User user)
        {
            if (ModelState.IsValid)
            {
                if (db.Users.FirstOrDefault(u => u.Username == user.Username) == null 
                    && db.Admins.FirstOrDefault(u => u.Username == user.Username) == null
                    && db.Conferences.FirstOrDefault(u => u.Username == user.Username) == null)
                {
